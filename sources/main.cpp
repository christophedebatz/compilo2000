...
void readGrammarFile(vector<string> *grammar, string fileName) {
    try {
        ...
        grammar->clear(); // on efface les donnees deja presentes
        string currentChar;
        do {
        ...
        cout << endl << " Chargement du fichier ./" << fileName << "..." << endl;
...
// fonction getPipedRulesInstructions
// entree: le vecteur contenant une chaque caractere d'une regle d'une grammaire
// sortie: un vecteur contenant chaque instruction de la règle donnée
...
vector<string> getFactorizableTerms (vector<string> pipedInstr, string pattern) {
    ...
    for (size_t instr = 0; instr < pipedInstr.size(); instr++) {
        if (pipedInstr[instr].substr(0, pattern.size()) == pattern) { // l'instruction actuelle contient le motif
            ...
            // sinon, l'instruction est terminée, on place epsilon
            else
...
vector<string> getNonFactorizableTerms (vector<string> pipedInstr, string pattern) {
    ...
    for (size_t instr = 0; instr < pipedInstr.size(); instr++) {
        if (pipedInstr[instr].substr(0, pattern.size()) != pattern) // si les x premiers caractères (depend de la longueur du motif) sont différents de ceux du motif
            otherPatterns.push_back(pipedInstr[instr]);
    }

    return otherPatterns; // on renvoi le vecteur contenant la liste des instructions non factorisables par le motif
}
...
// fonction donnant le motif le plus présent dans une expression de règle (c'est le motif que l'on utilisera pour factoriser, le facteur donc !)
...
string getRuleLeftFactorizationPattern (vector<string> pipedInstr) {
    ...
    cout << endl << endl << " ------ Dection du motif de factorisation ------" << endl;
    ...
    for (size_t instr = 0; instr < pipedInstr.size(); instr++) { // on prend chaque instruction de la regle
        for (unsigned letter = 0; letter <= pipedInstr[instr].size(); letter++) { // pour chaque instruction on prend chaque caractere
            ...
            cout << endl << " - Le motif " << currentPattern << " est present " << currentNbRep << " fois dans la regle." << endl;

            if (currentNbRep < globalNbRep) { // si on est allé trop loin, on retourne le motif d'avant
                cout << " On est alle trop loin, on prend le motif d'avant, soit " << globalPattern << endl << endl; // trace
                return globalPattern;
            }

            cout << " On peut tenter mieux...";

            globalNbRep = currentNbRep; // sinon on continu avec les valeurs actuelles comme nouvelles valeurs
            globalPattern = currentPattern;
        } // endfor
    } // endfor

    cout << endl << endl << " Le motif final est donc \"" << globalPattern << "\"." << endl; // trace
    string trace = (globalNbRep <= 1) ? "insuffisant. On a donc aucun motif pour cette regle." : "suffisant pour nous. On adopte le motif.";
    cout << " Ce motif ce retrouve dans " << globalNbRep << " instruction.\n Cela est " << trace << endl << endl;
...
bool makeLeftFactorization (vector<string> *grammar) {
    ...
    // on créer une copie de la grammaire pour que les comparaisons de longueur se basent tjrs sur le même vecteur
    // result_rules contient la grammaire resultante
    vector<string> rulesInstr, result_rules, grammar_copy = *grammar;
    string ruleLetter = grammar_copy[0];
    bool last = false; // cas du dernier element

    // on parcourt chaque caractère de la grammaire pour délimiter chaque regle
    for (size_t i = 0; i < grammar_copy.size(); i++)
    {
        if (grammar_copy[i] != ";" && i + 1 < grammar_copy.size() && !last) { // on est pas en fin de regle donc on insert les instructions
            if (i + 2 == (int)grammar_copy.size()) { // cas de l'avant dernier element
                rulesInstr.push_back(grammar_copy[i]);
                rulesInstr.push_back(grammar_copy[i + 1]); // on ajoute en même temps le dernier
                last = true;
            }
            else // cas des autres élement
                rulesInstr.push_back(grammar_copy[i]);
        }
        else { // on arrive en bout de regle, on traite la regle avant de passer à la suivante

            cout << endl << endl << " -------- NOUVELLE ANALYSE D'UNE REGLE --------" << endl << endl;

            vector<string> pipedInstr = getPipedRulesInstructions(&rulesInstr); // récupération des instructions de la regles
            string fPattern = getRuleLeftFactorizationPattern(pipedInstr); // récupération du motif de factorisation

            printVector(&pipedInstr, "Instruction de la regle en cours d'analyse");

            // s'il n'y a pas de motif avec plus de 1 lettre en commun, on ne factorise pas
            if (fPattern == "") {
                cout << endl << " Motif de factorisation inexistant... On recopie la regle originale!" << endl << endl;

                for (size_t t = 0; t < rulesInstr.size(); t++)
                    result_rules.push_back(rulesInstr[t]);
            }

            // cas où on peut factoriser
            else {

                vector<string> nonFactorizableTerms = getNonFactorizableTerms(pipedInstr, fPattern); // on recupere les non factorisables (beta)
                vector<string> factorizableTerms = getFactorizableTerms(pipedInstr, fPattern); // on récupère les factorisables (alpha)

                printVector(&nonFactorizableTerms, "Vecteur des termes NON factorisables"); // traces
                printVector(&factorizableTerms, "Vecteur des termes factorisables");

                // on commence à réecrire les nouvelles regles de factorisation
                result_rules.push_back(ruleLetter);

                // cas du motif de factorisation (nécessite un cast)
                for (unsigned p = 0; p < fPattern.size(); p++) {
                ...
                // on ajoute les termes non factorisables à la suite
                ...
                // fin de la première règle et début de la seconde
                result_rules.push_back(";");
                result_rules.push_back(ruleLetter + "'");
            ...
            result_rules.push_back(";"); // on termine la réécriture

            // petite "bidouille" pour savoir jusqu'a ou supprimer dans la grammaire
            short add = 1;
            if (grammar_copy[rulesInstr.size()] != ";")
                add = 0;

            // on supprime la regle traitée de la grammaire
            grammar->erase(grammar->begin(), grammar->begin() + rulesInstr.size() + add);

            // si la grammaire est vide c'est qu'il n'y a plus de regles à traiter, on termine
            if (grammar->empty())
                break;

            // sinon, on prend le nom de la regle suivante (la première lettre)
            else
                ruleLetter = grammar->at(1);

            // on supprime les instructions pour cette regle
            rulesInstr.clear();
        } // endif

    } // endfor

    // on insere les nouvelles regles dans la grammaire
    result_rules.pop_back();

    drawGrammar(&result_rules, "Liste des regles remplacantes au final"); // trace
...
int main()
{
    ...
    string temp;
    cout << endl << endl << " Appuyez sur une [ N ] puis valider pour commencer les tests... ";
    cin >> temp;


    vector<string> *grammar = new vector<string>(); // nouveau support de grammaire


    cout << endl << endl << " ---------------------------------------------------------" << endl << " ² SUPPRESSION DE LA RECURSIVITE GAUCHE DIRECTE" << endl << " ---------------------------------------------------------" << endl << endl;
    readGrammarFile(grammar, "recursivite1.txt"); // on charge les donnees
    drawGrammar(grammar, "Grammaire de base (avec recursivite gauche directe)");
    repareAllDirectLeftRecursivity(grammar);
    drawGrammar(grammar, "Grammaire traitee (sans recursivite gauche directe)");




    cout << endl << endl << " Appuyez sur une [ N ] puis valider pour la suite des tests... ";
    cin >> temp;

    cout << endl << endl << " ---------------------------------------------------------" << endl << " ² FACTORISATION GAUCHE" << endl << " ---------------------------------------------------------" << endl << endl;
    readGrammarFile(grammar, "factorisation1.txt"); // on charge un nouveau fichier
    drawGrammar(grammar, "Grammaire de base (sans factorisation gauche)");
    makeLeftFactorization(grammar);
    drawGrammar(grammar, "Grammaire traitee (factorisee a gauche)");
    ...
    cout << endl << endl << " ---------------------------------------------------------" << endl << " ² FACTORISATION GAUCHE" << endl << " ---------------------------------------------------------" << endl << endl;
    readGrammarFile(grammar, "factorisation2.txt"); // on charge un nouveau fichier
    drawGrammar(grammar, "Grammaire de base (sans factorisation gauche)");
    makeLeftFactorization(grammar);
    drawGrammar(grammar, "Grammaire traitee (factorisee a gauche)");
    ...
    cout << endl << endl << " -- MERCI ET A BIENTOT --\n\n Suppression donnees dynamiques..." << endl << endl;


    delete grammar; // supprime les données dynamiques
    return EXIT_SUCCESS;
}
